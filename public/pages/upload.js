...
Upload_template.innerHTML = `
	...
	<ul style="list-style-type:none;">
		...
		<li><mrp-file-button primary id="Upload_USPS">Upload USPS:</mrp-file-button></li>
		<li><mrp-file-button primary id="Upload_FX">Upload Fed EX:</mrp-file-button></li>
	</ul>
...
class UploadPage extends HTMLElement {
	constructor() {
		...
		EventBroker.listen("Upload_USPS_mrp-file-button_fileSelected", this, this.uploadUSPSData);
		EventBroker.listen("Upload_USPS_fileReady", this, this.addUSPSToDataBase);
		
		EventBroker.listen("Upload_FX_mrp-file-button_fileSelected", this, this.uploadFXData);
		EventBroker.listen("Upload_FX_fileReady", this, this.addFXToDataBase);
	}	
	...
	uploadUSPSData(comoponent,event){
		//******************NOT UPS******************************
		comoponent.triggerString = "Upload_USPS_fileReady";
		comoponent.excelToJSON(event.files[0]);
		//******************NOT UPS******************************
	}
	addUSPSToDataBase(comoponent,data){
		//******************NOT UPS******************************
		comoponent.setupAlert("Uploading USPS Data", "Uploading 1 out of of " + data.length.toString());
		addRecord(comoponent, data, 0);
		//******************NOT UPS******************************
		...
		async function addRecord(component, data, counter){
			...
			paredData['submitted'] = "";			
			paredData['ProNumber'] = "";			
			paredData['reasonForClaim'] = "";			
			
			paredData['ClaimNum'] = data[counter]['claimNum'];
			paredData['CarrierComment'] = data[counter]['status'];
			paredData['review'] = data[counter]['reviewDate'];
			paredData['submitted'] = data[counter]['submittedDate'];
			paredData['ProNumber'] = data[counter]['trackingNumber'];
			paredData['reasonForClaim'] = data[counter]['reasonForClaim'];

			//if(paredData['PRO Number'] !== ''){
				options.body=JSON.stringify(paredData);
			
				options.headers={'Content-Type':'application/json'}
				const response = await fetch('/api/usps',options);
	...
	uploadFXData(comoponent,event){
		//******************NOT UPS******************************
		comoponent.triggerString = "Upload_FX_fileReady";
		comoponent.excelToJSON(event.files[0]);
		//******************NOT UPS******************************
	}
	addFXToDataBase(comoponent,data){
		//******************NOT UPS******************************
		comoponent.setupAlert("Uploading Fed Ex Data", "Uploading 1 out of of " + data.length.toString());
		addRecord(comoponent, data, 0);
		//******************NOT UPS******************************
		...
		async function addRecord(component, data, counter){
			...
			paredData['ProNumber'] = "";
			paredData['CarrierComment'] = "no comment";
			paredData['deliveryDate'] = "";			
			paredData['shipDate'] = "";			
			paredData['shipperCity'] = "";
			paredData['recipiantCity'] = "";
			paredData['terminal'] = "";
			
			paredData['ProNumber'] = data[counter]['TRACKING NO. OR NICKNAME'];
			paredData['CarrierComment'] = data[counter]['STATUS'];
			paredData['deliveryDate'] = component.ExcelDateToJSDate(data[counter]['SCHEDULED DELIVERY DATE']);
			paredData['shipDate'] = component.ExcelDateToJSDate(data[counter]['SHIP DATE']);
			paredData['shipperCity'] = data[counter]['SHIPPER CITY, STATE'];
			paredData['recipiantCity'] = data[counter]['RECIPIENT CITY, STATE'];
			paredData['terminal'] = data[counter]['ORIGIN TERMINAL'];

			//if(paredData['PRO Number'] !== ''){
				options.body=JSON.stringify(paredData);
			
				options.headers={'Content-Type':'application/json'}
				const response = await fetch('/api/fx',options);
	...
	ExcelDateToJSDate(xlSerial){
		if(isNaN(xlSerial)){
			return xlSerial;
		}
		
		xlSerial = Math.trunc(xlSerial)

		debugger;
		var newDate = new Date((xlSerial - (25567 + 2))*86400*1000);
